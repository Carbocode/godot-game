...
func _notification(what):
	if what == NOTIFICATION_PREDELETE and context:
		context.free()
...
func _update(compute_list : int, cascade_index : int, parameters : Array[WaveCascadeParameters]) -> void:
	...
	if params.should_generate_spectrum:
		...
		var push_constants = RenderingContext.create_push_constant([
			params.spectrum_seed.x, 
			params.spectrum_seed.y,
			params.tile_length.x, 
			params.tile_length.y,
			alpha, 
			omega, 
			params.wind_speed,
			deg_to_rad(params.wind_direction), 
			DEPTH,
			params.swell, 
			params.detail, 
			params.spread,
		...
		pipelines[&'spectrum_compute'].call(context, compute_list, push_constants)
		params.should_generate_spectrum = false
	
	var mod_push = RenderingContext.create_push_constant([params.tile_length.x, params.tile_length.y, DEPTH, params.time, cascade_index])
	pipelines[&'spectrum_modulate'].call(context, compute_list, mod_push)
	...
	var unpack_push = RenderingContext.create_push_constant([cascade_index, params.whitecap, params.foam_grow_rate, params.foam_decay_rate])
	pipelines[&'fft_unpack'].call(context, compute_list, unpack_push)
...
func init_gpu(num_cascades : int) -> void:
	...
	var spectrum_compute_shader := context.load_shader('res://assets/shaders/ocean/compute/spectrum_compute.glsl')
	var fft_butterfly_shader := context.load_shader('res://assets/shaders/ocean/compute/fft_butterfly.glsl')
	var spectrum_modulate_shader := context.load_shader('res://assets/shaders/ocean/compute/spectrum_modulate.glsl')
	var fft_compute_shader := context.load_shader('res://assets/shaders/ocean/compute/fft_compute.glsl')
	var transpose_shader := context.load_shader('res://assets/shaders/ocean/compute/transpose.glsl')
	var fft_unpack_shader := context.load_shader('res://assets/shaders/ocean/compute/fft_unpack.glsl')
